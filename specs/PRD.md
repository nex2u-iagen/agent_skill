...
**Por que agora:**
A ascensão de LLMs super eficientes (Gemini 2.5/3.0+, DeepSeek, groq e ollama) e a facilidade da API do Telegram permitem rodar um agente pessoal sem os atritos operacionais de UI na web.
...
- [ ] G-02: Intercambiar "cérebros" (LLMs) usando padronização (DeepSeek, Gemini e groq).
...
| RF-03 | O sistema deve alternar "LLMs" instanciando fábricas (`ProviderFactory`) | Must | Trocar "gemini" por "deepseek" ou "groq" no config envia prompts diretos ao endpoint alvo corretamente parseado. |
...
2. O AgentLoop tenta fallback (outras APIs) no config ou falha graciosamente enviando aviso pro Telegram em vez de quebrar a Promise da main engine.
